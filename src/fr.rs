...
#[repr(transparent)]
pub struct Fr(pub(crate) blst::blst_fr);
...
impl From<u64> for Fr {
    fn from(val: u64) -> Fr {
        Fr(blst::blst_fr { l: [val, 0, 0, 0] }) * R2
    }
}
...
pub const MODULUS: Fr = Fr(blst::blst_fr { l: [
...
const GENERATOR: Fr = Fr(blst::blst_fr { l: [
...
const ROOT_OF_UNITY: Fr = Fr(blst::blst_fr { l: [
...
impl Fr {
    ...
    pub fn from_bytes(bytes: &[u8; 32]) -> CtOption<Fr> {
        let mut tmp = Fr(blst::blst_fr { l: [0, 0, 0, 0] });

        tmp.0.l[0] = u64::from_le_bytes(bytes[0..8].try_into().unwrap());
        tmp.0.l[1] = u64::from_le_bytes(bytes[8..16].try_into().unwrap());
        tmp.0.l[2] = u64::from_le_bytes(bytes[16..24].try_into().unwrap());
        tmp.0.l[3] = u64::from_le_bytes(bytes[24..32].try_into().unwrap());

        // Try to subtract the modulus
        let (_, borrow) = sbb(tmp.0.l[0], MODULUS.0.l[0], 0);
        let (_, borrow) = sbb(tmp.0.l[1], MODULUS.0.l[1], borrow);
        let (_, borrow) = sbb(tmp.0.l[2], MODULUS.0.l[2], borrow);
        let (_, borrow) = sbb(tmp.0.l[3], MODULUS.0.l[3], borrow);
    ...
    pub fn to_bytes(&self) -> [u8; 32] {
        ...
        // (a.R) / R = a
        let tmp = Fr::montgomery_reduce(self.0.l[0], self.0.l[1], self.0.l[2], self.0.l[3], 0, 0, 0, 0);

        let mut res = [0; 32];
        res[0..8].copy_from_slice(&tmp.0.l[0].to_le_bytes());
        res[8..16].copy_from_slice(&tmp.0.l[1].to_le_bytes());
        res[16..24].copy_from_slice(&tmp.0.l[2].to_le_bytes());
        res[24..32].copy_from_slice(&tmp.0.l[3].to_le_bytes());
    ...
    fn from_u512(limbs: [u64; 8]) -> Fr {
        ...
        // constant `R2` or `R3`.
        let d0 = Fr(blst::blst_fr { l: [limbs[0], limbs[1], limbs[2], limbs[3]] });
        let d1 = Fr(blst::blst_fr { l: [limbs[4], limbs[5], limbs[6], limbs[7]] });
    ...
    pub const fn from_raw(val: [u64; 4]) -> Self {
        (&Fr(blst::blst_fr { l: val })).mul(&R2)
    }

    /// Squares this element.
    #[inline]
    pub const fn square(&self) -> Fr {
        let (r1, carry) = mac(0, self.0.l[0], self.0.l[1], 0);
        let (r2, carry) = mac(0, self.0.l[0], self.0.l[2], carry);
        let (r3, r4) = mac(0, self.0.l[0], self.0.l[3], carry);

        let (r3, carry) = mac(r3, self.0.l[1], self.0.l[2], 0);
        let (r4, r5) = mac(r4, self.0.l[1], self.0.l[3], carry);

        let (r5, r6) = mac(r5, self.0.l[2], self.0.l[3], 0);
        ...
        let (r0, carry) = mac(0, self.0.l[0], self.0.l[0], 0);
        let (r1, carry) = adc(0, r1, carry);
        let (r2, carry) = mac(r2, self.0.l[1], self.0.l[1], carry);
        let (r3, carry) = adc(0, r3, carry);
        let (r4, carry) = mac(r4, self.0.l[2], self.0.l[2], carry);
        let (r5, carry) = adc(0, r5, carry);
        let (r6, carry) = mac(r6, self.0.l[3], self.0.l[3], carry);
    ...
    const fn montgomery_reduce(
        ...
        (&Fr(blst::blst_fr { l: [r4, r5, r6, r7] })).sub(&MODULUS)
    }
    ...
    pub const fn mul(&self, rhs: &Self) -> Self {
        ...
        let (r0, carry) = mac(0, self.0.l[0], rhs.0.l[0], 0);
        let (r1, carry) = mac(0, self.0.l[0], rhs.0.l[1], carry);
        let (r2, carry) = mac(0, self.0.l[0], rhs.0.l[2], carry);
        let (r3, r4) = mac(0, self.0.l[0], rhs.0.l[3], carry);

        let (r1, carry) = mac(r1, self.0.l[1], rhs.0.l[0], 0);
        let (r2, carry) = mac(r2, self.0.l[1], rhs.0.l[1], carry);
        let (r3, carry) = mac(r3, self.0.l[1], rhs.0.l[2], carry);
        let (r4, r5) = mac(r4, self.0.l[1], rhs.0.l[3], carry);

        let (r2, carry) = mac(r2, self.0.l[2], rhs.0.l[0], 0);
        let (r3, carry) = mac(r3, self.0.l[2], rhs.0.l[1], carry);
        let (r4, carry) = mac(r4, self.0.l[2], rhs.0.l[2], carry);
        let (r5, r6) = mac(r5, self.0.l[2], rhs.0.l[3], carry);

        let (r3, carry) = mac(r3, self.0.l[3], rhs.0.l[0], 0);
        let (r4, carry) = mac(r4, self.0.l[3], rhs.0.l[1], carry);
        let (r5, carry) = mac(r5, self.0.l[3], rhs.0.l[2], carry);
        let (r6, r7) = mac(r6, self.0.l[3], rhs.0.l[3], carry);
    ...
    pub const fn sub(&self, rhs: &Self) -> Self {
        let (d0, borrow) = sbb(self.0.l[0], rhs.0.l[0], 0);
        let (d1, borrow) = sbb(self.0.l[1], rhs.0.l[1], borrow);
        let (d2, borrow) = sbb(self.0.l[2], rhs.0.l[2], borrow);
        let (d3, borrow) = sbb(self.0.l[3], rhs.0.l[3], borrow);

        // If underflow occurred on the final limb, borrow = 0xfff...fff, otherwise
        // borrow = 0x000...000. Thus, we use it as a mask to conditionally add the modulus.
        let (d0, carry) = adc(d0, MODULUS.0.l[0] & borrow, 0);
        let (d1, carry) = adc(d1, MODULUS.0.l[1] & borrow, carry);
        let (d2, carry) = adc(d2, MODULUS.0.l[2] & borrow, carry);
        let (d3, _) = adc(d3, MODULUS.0.l[3] & borrow, carry);

        Fr(blst::blst_fr { l: [d0, d1, d2, d3] })
    }

    /// Adds this element to another element.
    #[inline]
    pub const fn add(&self, rhs: &Self) -> Self {
        let (d0, carry) = adc(self.0.l[0], rhs.0.l[0], 0);
        let (d1, carry) = adc(self.0.l[1], rhs.0.l[1], carry);
        let (d2, carry) = adc(self.0.l[2], rhs.0.l[2], carry);
        let (d3, _) = adc(self.0.l[3], rhs.0.l[3], carry);

        // Attempt to subtract the modulus, to ensure the value
        // is smaller than the modulus.
        (&Fr(blst::blst_fr { l: [d0, d1, d2, d3] })).sub(&MODULUS)
    }
    ...
    pub const fn neg(&self) -> Self {
        ...
        // be in the field.
        let (d0, borrow) = sbb(MODULUS.0.l[0], self.0.l[0], 0);
        let (d1, borrow) = sbb(MODULUS.0.l[1], self.0.l[1], borrow);
        let (d2, borrow) = sbb(MODULUS.0.l[2], self.0.l[2], borrow);
        let (d3, _) = sbb(MODULUS.0.l[3], self.0.l[3], borrow);

        // `tmp` could be `MODULUS` if `self` was zero. Create a mask that is
        // zero if `self` was zero, and `u64::max_value()` if self was nonzero.
        let mask = (((self.0.l[0] | self.0.l[1] | self.0.l[2] | self.0.l[3]) == 0) as u64).wrapping_sub(1);

        Fr(blst::blst_fr { l: [d0 & mask, d1 & mask, d2 & mask, d3 & mask] })
    }
}
...
fn test_from_raw() {
    ...
    assert_eq!(Fr::from_raw(MODULUS.0.l), Fr::zero());
